...
*/
int stock_trend_scraper(char symbol[20])
{
    ...
    snprintf(url, sizeof(url), "https://query1.finance.yahoo.com/v8/finance/chart/%s.NS?interval=1d&range=15d",symbol); // adjusting api endpoint according to stock

    fp=fopen("trend_data.txt","w"); // file in which fetched data would be written
